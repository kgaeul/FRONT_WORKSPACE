...
const images=[
'./pagination/사진/에몽가1.jpg',
'./pagination/사진/에몽가2.jpg',
'./pagination/사진/에몽가3.jpg',
'./pagination/사진/에몽가4.jpg',
'./pagination/사진/에몽가5.jpg',
];

const imagePerpage = 2; //한 페이지에 표시될 이미지 수
let currentPage = 0; //현재 페이지를 나타내는 변수
//초기값을 0으로 설정하여 첫 번째 페이지를 표시함
...
호출 될 때 마다 페이지를 전환하고 해당 페이지의 이미지를 표시하는 데 사용
...
//현재 페이지 번호('page')와 페이지당 이미지 수(imagePerpage)를 곱해서 시작 인덱스를 계산
// 페이지 내에서 표시할 이미지의 첫 번째 이미지 인덱스를 나타냄
const startIndex = page * imagePerpage;
...
//pageImages 배열에 저장 이 배열은 현재 페이지에 표시될 이미지들을 담고 있음
const pageImages = images.slice(startIndex, endIndex);

imageContainer.innerHTML=''; //이미지를 표시해 줄 컨테이너

pageImages.forEach(images => {
    const imgElement = document.createElement('img'); //각 이미지를 표시하기 위한 <img> 태그 요소 생성
   //<img> 태그에 src 속성을 사진(현재 이미지의 파일 경로)으로 설정
...
//이전 버튼 활성화 비활성화 시키는 기능을 나타냄 첫 페이지일 경우 이전 버튼을 비활성화 시켜서 첫 페이지에서 이전 버튼 클릭하지 못하게 막음
prevButton.disabled = currentPage ===0;

/*
(currentPage+1) * imagePerpage = 다음 페이지에 해당하는 이미지의 첫번째 인덱스
images.length = <images>배열의 길이로, 페이지를 넘어갈 수 있는 이미지의 전체 수를 나타낸다. 

*/

//다음 버튼에서 마지막 페이지일 경우 이미지가 없을 때 더 이상 다음버튼이 눌리는 것을 방지하기 위해서 다음 버튼 또한 클릭하지 못하게 막음
nextButton.disabled = (currentPage+1) * imagePerpage >= images.length;
}

//이전 버튼에 대한 클릭 이벤트
//사용자가 이전 버튼을 클릭할 때 이 함수가 실행됨
prevButton.addEventListener('click',()=> {//현재 페이지 번호가 0보다 클 경우에만 실행
if(currentPage > 0){
    currentPage --; // 현재 페이지 번호에서 하나를 감소시킴
    displayImg(currentPage); //새로운 현재 페이지에 해당하는 이미지를 표시
    // 이전 버튼이 첫 번째 페이지에서 클릭하게 되면 더 이상 눌리지 않게 비활성화 시킴
    updateButtons(); 
}
});
...
*/
if((currentPage + 1) *imagePerpage < images.length){
